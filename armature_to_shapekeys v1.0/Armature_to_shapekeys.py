...
class ArmatureToShapekeys(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        modArmExists = bool(obj.modifiers["Armature"].object)
        shapeKeysEmpty = bool(bpy.context.object.data.shape_keys)
        ...
        if(modifierExists == True and modArmExists == True):
            ...
            bpy.ops.object.modifier_apply_as_shapekey(keep_modifier=True, modifier="Armature", report=False)
            
            index = bpy.context.object.data.shape_keys.key_blocks
            
            print("starting ShapeKey Loop" + "\n" )
            
            if(index.find("Armature") != 0):
                x = 0
                prior = bpy.context.scene.frame_current
                print("Armature ShapeKey found")
                bpy.data.scenes['Scene'].frame_current = 1
                
                while(cframe<=maxframes) and  (x<=keys): 
                    x=x+1
                    print("cFrame is in Range")
                    
                    index["Armature"].name = str(cframe)
                    bpy.context.object.active_shape_key_index = index.find(str(cframe))
                    ...
                    if(bpy.context.object.active_shape_key_index == "1"):
                        ...
                        if(prior != cframe):
                            bpy.context.object.active_shape_key_index = index.find(str(prior))
                    ...
                    bpy.ops.object.modifier_apply_as_shapekey(keep_modifier=True, modifier="Armature", report=False)
                    bpy.context.object.active_shape_key_index = index.find("Armature")
                    bpy.context.scene.frame_current = prior
